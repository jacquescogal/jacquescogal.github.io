...
const CGP = ({speed=10,preoccupy=true,quickLeave=true,className,onClick,style,onAnimationEnd,randomChoice=5,children}) => {
    ...
    useEffect(()=>{
        const timeout=setTimeout(()=>{
            if (deleteLock){
            if (text.length>0 && text.slice(0)===children.slice(0,text.length)){
                setDeleteLock(false)
                if (countDown==0) setCountdown(randomChoice);
                else if (countDown>0) setCountdown(0);
            }
    ...
    useEffect(()=>{
        const timeout=setTimeout(()=>{
            if (deleteLock==false && text!==children && text.length<=children.length) {
                if (text.length>0 && text.slice(0,-1)!==children.slice(0,text.length-1)) {
                    setText(children.slice(0,text.length));
                }
                if (countDown>0) {
                    ...
                    if (text.length>0) setText(text.substring(0,text.length-1).concat(children[text.length-1]))
                    else setText(children[0])
  ...
  return (
    ...
    <p className={className+((preoccupy)?' invisible h-0':' hidden')} style={style}>{children}</p>
    </div>
  )
}
