...
const Intro = ({setIntroRef}) => {
  ...
  const imageLoadHandler=()=>{
    setGreetings(greetings.slice(1,-1))
    return ()=>clearTimeout(timeout)
  }
  ...
  return (
    ...
    {
      greetings.map((mi)=>
      <div key={mi.key} className={'absolute transition transform ease-in-out duration-500 w-[100px] h-[40px] bg-white border-black border-2 rounded-lg cursor-default'} 
      style={{top: `${mi.y}px`, left: `${mi.x}px`}}
      >
          <p className={introStyle['dialogue']} onMouseLeave={()=>setGreetings(greetings.slice(1,-1))} > {mi.text}</p>
      </div>
  )
    }
    </CSSTransitionGroup>
    ...
    <TGP toGenerate={"//"+imageCaption} className='text-left text-green-200 opacity-100' speed={1}/> <span className={"transition ease-out duration-100 relative flex h-3 w-3 mx-1 "+((inImage)?"opacity-0":"opacity-100")} >
  ...
  <span className="relative inline-flex inset-y-1/2 rounded-full h-3 w-3 bg-green-400"></span>
      ...
      <div  className="px-10 py-10 w-100 flex-none">
        ...
        <div className='flex flex-row'>
          <MouseSVG className='fill-white h-20 w-20 self-end'/>
